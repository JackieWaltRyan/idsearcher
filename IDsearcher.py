                        ...
                        try:
                            ...
                            if len(data_raw) > 0:
                                ...
                                if settings["Format"]["TXT"]:
                                    ...
                                    trigger = create_file_out(cat=cat,
                                                              file="txt",
                                                              data=data_txt) if trigger else False

                                if settings["Format"]["JSON"]:
                                    trigger = create_file_out(cat=cat,
                                                              file="json",
                                                              data=data_json) if trigger else False
                                ...
                                if settings["Format"]["XML"]:
                                    ...
                                    trigger = create_file_out(cat=cat,
                                                              file="xml",
                                                              data=data_xml) if trigger else False
...
def load_files():
    try:
        ...
        if len(files) > 0:
            return parse_gameobjectdata(file=files[0],
                                        folder="")
        else:
            if exists(path="000_and_mlpextra_common/gameobjectdata.xml"):
                return parse_gameobjectdata(file="gameobjectdata.xml",
                                            folder="000_and_mlpextra_common/")
            else:
